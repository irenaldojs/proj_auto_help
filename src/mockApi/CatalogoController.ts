import { WebAPISimulator } from "../Objective-UI";
import { CatalogoItem } from "../viewModels/CatalogoItem";
import { RetornoAPICatalogo } from "../viewModels/RetornoAPICatalogo";
import { Veiculo } from "../viewModels/Veiculo";

export class CatalogoController extends WebAPISimulator
{
    private static DATA_MOCK: Array<CatalogoItem> = null;
    ...
    private listarModelos(parametros: Array<string>): Array<string> {
        ...
        for (var c = 0; c < dados.length; c++) {
            var item: CatalogoItem = dados[c];
            for (var v = 0; v < item.aplicacao.length; v++) {
                var veiculo: Veiculo = item.aplicacao[v];
    ...
    private listarFabricantes(): Array<string> {
        var dados = CatalogoController.DATA_MOCK;
        var retorno: Array<string> = [];
        for (var c = 0; c < dados.length; c++) {
            var item: CatalogoItem = dados[c];
    ...
    private listarMarcas(): Array<string> {
        var dados = CatalogoController.DATA_MOCK;
        var retorno: Array<string> = [];
        for (var c = 0; c < dados.length; c++) {
            var item: CatalogoItem = dados[c];
    ...
    private buscaCatalogo(filtros: Array<string>): Array<RetornoAPICatalogo> {
        var marca = filtros[1];
        var fabricante = filtros[2];
        var modelo = filtros[3];

        var dados = CatalogoController.DATA_MOCK;
        var retorno: Array<RetornoAPICatalogo> = [];

        for (var i = 0; i < dados.length; i++) {
            var item: CatalogoItem = dados[i];
            if (item.marca == marca) {
                for (var a = 0; a < item.aplicacao.length; a++) {
                    var veiculo: Veiculo = item.aplicacao[a];
                    if (veiculo.fabricante == fabricante ||
